...
module.exports = function ($scope, $state, businessFactory, socketService, $ionicSideMenuDelegate,
  ...
  $scope.currAppointment = $state.params.appointment;
  ...
  $scope.switchState = function (state) {
    if (state == 'reschedule') {
      $scope.selectedDate = new Date($scope.currAppointment.start.date);
    }
  ...
  businessFactory.serviceDetails($scope.currAppointment.service)
    .then(function (data) {
      ...
      if ($scope.currAppointment.employee._id) {
        $scope.employee = window.underscore.findWhere($scope.service.employees, {_id: $scope.currAppointment.employee._id});
      } else {
        $scope.employee = window.underscore.findWhere($scope.service.employees, {_id: $scope.currAppointment.employee});
      }
  ...
  var dateSelected = moment().set({
    'date': moment(new Date($scope.currAppointment.start.date)).date(),
    'month': moment(new Date($scope.currAppointment.start.date)).month(),
    'year': moment(new Date($scope.currAppointment.start.date)).year(),
    'hour': moment($scope.currAppointment.start.time, 'hh:mm a').hour(),
    'minute': moment($scope.currAppointment.start.time, 'hh:mm a').minute(),
    'second': '00'
  });
  var today = moment();
  ...
  $scope.$watch('selectedDate', function (newVal, oldVal) {
    if (newVal) {
      ...
      //var roomId = moment(selectedDate).format('MM/YYYY') + $scope.currAppointment.employee._id;
      var employeeId;
  ...
  function getAvailableTimes(date, employeeId) {
    ...
    if ($scope.currAppointment.customer !== null) {
      employeeApptObj.customerId = $scope.currAppointment.customer._id;
    }
    ...
    userFactory.getAppts(employeeApptObj)
      .then(function (appointmentsArray) {
        ...
        var testTime = function (element, index, list) {
          if (element.time === $scope.currAppointment.start.time && !$scope.activeTime) {
            $scope.$broadcast('timer-clear');
            $scope.showCount = false;
            ...
            $scope.availableTimes[index].toggled = true;
            $scope.selectedIndex = index;
          } else if (element.time === $scope.currAppointment.start.time && $scope.activeTime) {
            $scope.availableTimes[index].available = true;
            $scope.availableTimes[index].status = false;
            $scope.availableTimes[index].toggled = true;
          }
        ...
        //in available timess
        if ($scope.currAppointment !== 'canceled' && $scope.newRoomDate === $scope.currAppointment.start.date) {
  ...
  var checkShowUpdate = function (timeObj) {
    ...
    if (moment(new Date($scope.selectedDate)).date() !== moment(new Date($scope.currAppointment.start.date)).date()) {
      if (!$scope.datePassed) {
        $scope.showUpdate = true;
      }
    } else if (moment(new Date($scope.selectedDate)).date() === moment(new Date($scope.currAppointment.start.date)).date()) {
      if (timeObj.time !== $scope.currAppointment.start.time && !$scope.datePassed) {
        $scope.showUpdate = true;
      }
    }
  };
  ...
  $scope.createAppointmentObj = function (timeObj, index) {
    ...
    if (!timeStarted && $scope.currAppointment.start.time !== timeObj.time) {
      $scope.$broadcast('timer-start');
      $scope.timerRunning = true;
      timeStarted = true;
      //if it's already running reset it then start it
    } else if (timeStarted && $scope.currAppointment.start.time !== timeObj.time) {
    ...
    $scope.appointment = {
      _id: $scope.currAppointment._id,
      businessId: $scope.currAppointment.businessId,
      employee: $scope.currAppointment.employee,
      start: {
        date: apptDate,
    ...
    if (typeof($scope.currAppointment.customer) === 'string') {
      $scope.appointment.customer = $scope.currAppointment.customer;
  ...
  $scope.confirmCancel = function () {
    ...
    confirmCancelPopup.then(function (res) {
      if (res) {
        ...
        businessFactory.cancelAppointment($scope.currAppointment)
          .then(function () {
            notifyCancel($scope.currAppointment);
            var socketData = {
              'from': $rootScope.currentUser._id,
              'appointment': $scope.currAppointment,
              'roomId': $scope.newRoomDate.toString() + $scope.employee._id
            };
  ...
  function notifyReschedule(appointment, rescheduled) {
    var customerNotification = 'Your ' + $scope.service.name + ' scheduled for  ' + moment($scope.currAppointment.start.full).format('MMM Do YYYY, h:mm a')
      + ' was rescheduled to ';
    var employeeNotification = 'Your ' + $scope.service.name + ' scheduled for ' + moment($scope.currAppointment.start.full).format('MMM Do YYYY, h:mm a')
      + ' was rescheduled to ';

    if (rescheduled) {
      employeeNotification = 'Your request to reschedule ' + $scope.service.name + ' originally scheduled for ' + moment($scope.currAppointment.start.full).format('MMM Do YYYY, h:mm a')
